...
export default function CategoryManager({ categories, onChange }: CategoryManagerProps) {
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      <div>
        <h2 className="text-[10px] font-medium text-neutral-400 dark:text-neutral-500 tracking-[0.2em] uppercase">Kategori</h2>
        <p className="text-xs text-neutral-450 dark:text-neutral-400 mt-0.5">Tambah atau edit kategori proyek.</p>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-1 bg-white dark:bg-[#0d0d0f]/50 border border-neutral-200/60 dark:border-neutral-900/40 rounded-xl p-6 shadow-xs h-fit space-y-5">
          <div className="flex items-start justify-between">
            <div>
              <span className="text-xs font-semibold text-neutral-850 dark:text-neutral-200 block">{editing ? "Edit Kategori" : "Tambah Kategori"}</span>
            ...
            {editing && <button onClick={cancelEdit} className="p-1.5 text-neutral-400 hover:text-neutral-600 dark:hover:text-neutral-200 rounded-lg bg-neutral-100 dark:bg-neutral-900 transition-all cursor-pointer"><X className="w-3.5 h-3.5" /></button>}
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="space-y-2.5">
              ...
              <div className="grid grid-cols-4 gap-2 border border-neutral-200/50 dark:border-neutral-850/50 p-3 rounded-lg bg-neutral-50/50 dark:bg-[#121214]/20">
                {AVAILABLE_ICONS.map((ico) => (
                  <button key={ico.name} type="button" onClick={() => setSelectedIcon(ico.name)} title={ico.label}
                    className={`p-2.5 rounded-lg border flex items-center justify-center transition-all cursor-pointer ${
                      selectedIcon === ico.name ? "bg-neutral-950 dark:bg-neutral-100 border-transparent text-white dark:text-neutral-950 scale-105 shadow-xs" : "bg-white dark:bg-[#0d0d0f]/60 border-neutral-200/70 dark:border-neutral-900 text-neutral-400 hover:text-neutral-800 dark:hover:text-white"
                    }`}>
                    {renderIcon(ico.name)}
                  </button>
                ))}
            ...
            <button type="submit" className="w-full flex items-center justify-center gap-1.5 py-2.5 text-xs font-semibold rounded-lg bg-neutral-950 dark:bg-neutral-100 hover:bg-neutral-850 dark:hover:bg-neutral-200 text-white dark:text-neutral-950 transition-all cursor-pointer">
              {editing ? <><Check className="w-3.5 h-3.5" />Simpan</> : <><Plus className="w-3.5 h-3.5" />Tambah</>}
            </button>
            {editing && <button type="button" onClick={cancelEdit} className="w-full py-2.5 text-xs font-medium rounded-lg text-neutral-500 hover:text-neutral-800 dark:hover:text-neutral-200 transition-all cursor-pointer border border-neutral-200 dark:border-neutral-800">Batal</button>}
          </form>
        </div>

        <div className="lg:col-span-2 space-y-4">
          <span className="text-xs font-semibold text-neutral-850 dark:text-neutral-200 block">Semua Kategori ({categories.length})</span>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {categories.map((cat) => (
              <div key={cat.id} className="bg-white dark:bg-[#0d0d0f]/30 border border-neutral-200/60 dark:border-neutral-900/40 rounded-xl p-5 flex items-center justify-between group transition-all hover:border-neutral-350 dark:hover:border-neutral-800">
                <div className="flex items-center gap-3.5 min-w-0">
                  <div className="w-9 h-9 rounded-lg bg-neutral-50 dark:bg-neutral-900 border border-neutral-150 dark:border-neutral-850 flex items-center justify-center text-neutral-600 dark:text-neutral-300">
                ...
                <div className="flex items-center gap-1.5">
                  <button onClick={() => startEdit(cat)} className={`p-2 rounded-lg transition-all cursor-pointer ${editing?.id === cat.id ? "bg-neutral-950 dark:bg-neutral-100 text-white dark:text-neutral-950 scale-105 shadow-xs" : "text-neutral-400 hover:text-neutral-800 dark:hover:text-white hover:bg-neutral-50 dark:hover:bg-neutral-900"}`}>
                  ...
                  <button onClick={() => setDeleteTarget(cat)} className="p-2 text-neutral-400 hover:text-red-500 rounded-lg hover:bg-neutral-50 dark:hover:bg-neutral-900 transition-all cursor-pointer">
      ...
      {deleteTarget && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white dark:bg-[#0d0d0f] border border-neutral-200/70 dark:border-neutral-900 rounded-lg w-full max-w-sm p-6 sm:p-7 shadow-2xl animate-fade-in">
            <div className="flex items-start gap-3.5">
              ...
              <div className="flex-1">
                <h3 className="text-xs font-bold text-neutral-900 dark:text-white">Hapus Kategori?</h3>
                <p className="mt-1.5 text-[11px] text-neutral-450 dark:text-neutral-400 leading-relaxed">
            ...
            <div className="mt-5 flex items-center justify-end gap-2 border-t border-neutral-100 dark:border-neutral-900/60 pt-3.5">
              <button onClick={() => setDeleteTarget(null)} className="px-3 py-1.5 border border-neutral-200 dark:border-neutral-850 text-neutral-500 rounded-md hover:bg-neutral-50 dark:hover:bg-neutral-900 transition-all text-[11px] font-medium cursor-pointer">Batal</button>
